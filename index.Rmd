...
corpus <- corpus_raw %>% select(
  energy, valence, track.popularity, track.artists, track.name
)
```

```{r lyrics, include=FALSE}
#! load lyrics
...
by(corpus, seq_len(nrow(corpus)), function(row) {
...
```{r sentiment}
#! add lyrical sentiment values to corpus

lyrics_db <- jsonlite::fromJSON(LYRICS_FILE)

# corpus$lyrics_sentiment <- lyrics_db$sentiment
nrows(corpus)
```
...
Furthermore, if we roughly break down music into four main elements, we get *instrumentation*, *melody*, *harmony* and *rhythm*. We will investigate each separately, and in the end combined.
...
```{r}
ggplot(corpus, aes(valence, energy, size=track.popularity)) +
  labs(title="Energy vs. Valence", x="Valence", y="Energy") +
  geom_point()
```
